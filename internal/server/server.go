...
func New(db *store.DB) *Server {
	...
	s.mux.HandleFunc("GET /api/workflows", s.list)
	s.mux.HandleFunc("POST /api/workflows", s.create)
	s.mux.HandleFunc("GET /api/workflows/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/workflows/{id}", s.del)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"status": "ok", "service": "stockyard-escrow", "time": time.Now().UTC().Format(time.RFC3339)})
}

func (s *Server) stats(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"workflows": s.db.Count()})
}

func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	items := s.db.List()
	if items == nil { items = []store.Workflow{} }
	wj(w, map[string]any{"workflows": items, "count": len(items)})
}

func (s *Server) create(w http.ResponseWriter, r *http.Request) {
	var e store.Workflow
